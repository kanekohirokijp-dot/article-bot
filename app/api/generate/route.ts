...
- 요리명・메뉴명도 한국어로 표기할 것
- 일본어 낫표（「」『』）를 절대 사용하지 말 것
  - 역명・고유명사는 괄호 없이 그대로 쓰거나, 강조가 필요한 경우 작은따옴표（'시부야역'）를 사용할 것
...
- 基本情報セクションは以下のコンパクト形式で出力すること：
  **기본 정보 📍**
  ...
  - 액세스: （アクセス）
  - 결제: （支払方法）
  - 가게 인스타그램: [アカウント名](https://www.instagram.com/アカウント名/)（ある場合のみ）
  - 공식 사이트: [공식 사이트](URL)（ある場合のみ）
  ルール：
  - 各項目は「- 項目名: 値」の1行に収める（項目名と値の間に改行しない）
  - 項目間に空行を入れない
  - 項目名に**ボールド**を使わない
  - 店舗Instagramのラベルはかならず「가게 인스타그램:」とすること（LOCAL STANDARD TOKYOのInstagramと区別するため）
  - 公式サイトは [공식 사이트](URL) 形式
  - 情報がない項目は省略すること（「요확인」と書かない）
- 全体の長さ：スクロール2〜3画面分
...
${photoUrl?.trim() ? `\n【写真引用（Instagram紹介セクションの直後、ハッシュタグの直前に必ず追加すること）】\n사진 인용：[Google Maps](${photoUrl.trim()})` : ""}`;
